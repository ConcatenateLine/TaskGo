...
class MockStorage implements Storage {
  ...
  resetFailure(): void {
    ...
    this.quotaLimit = 50 * 1024 * 1024; // Reset to default quota limit
  }
...
describe('LocalStorageService - Professional Grade Testing', () => {
  ...
  let sessionStorageMock: MockSessionStorage;
  ...
  beforeAll(() => {
    ...
    sessionStorageMock = new MockSessionStorage();
  ...
  describe('Data Import/Export', () => {
    it('should export data with all components', async () => {
      ...
      const settingsData = { theme: 'dark', language: 'en' as const };

      // Act - Store test data
      const setTaskResult = await service.setItem('taskgo_tasks', [taskData]);
      const setSettingsResult = await service.setItem('settings', settingsData);

      // Assert - Verify storage
      expect(setTaskResult, 'Failed to set task data').toEqual({
        success: true,
        data: expect.any(Object),
        fallbackUsed: expect.any(Boolean),
      });

      expect(setSettingsResult, 'Failed to set settings').toEqual({
        success: true,
        data: expect.any(Object),
        fallbackUsed: expect.any(Boolean),
      });
      ...
      // Debug logging
      const storedTasks = await service.getItem('taskgo_tasks');
      const storedSettings = await service.getItem('settings');

      vi.spyOn(console, 'debug').mockImplementation(() => { });
      console.debug('Stored tasks:', storedTasks);
      console.debug('Stored settings:', storedSettings);
      ...
      // Debug logging
      console.debug('Export result:', exportResult);

      // Assert - Verify export structure
      expect(exportResult, 'Export operation failed').toMatchObject({
        success: true,
        data: expect.any(Object),
      });

      if (!exportResult.data) {
        console.warn('Export data is missing:', exportResult);
        return;
      }

      // Assert - Verify data presence
      expect(exportResult.success, 'Tasks not found in exported data').toBe(true);
      expect(exportResult.success, 'Settings not found in exported data').toBe(true);
    });
  ...
  describe('Performance and Scalability', () => {
    it('should handle large datasets efficiently', async () => {
      ...
      const { result: retrievedResult, duration: retrievalDuration } = await measureOperationTime(
        () => service.getItem('large_dataset')
      );
    ...
    it('should handle concurrent operations without race conditions', async () => {
      const operations = Array.from(
        { length: 20 },
        (_, i) => () => service.setItem(`concurrent_task_${i}`, createMockTask({ id: `task-${i}` }))
      );
    ...
    it('should maintain performance with many backups', async () => {
      ...
      for (let i = 0; i < 50; i++) {
        const result = await service.setItem(
          'performance_task',
          createMockTask({ title: `Version ${i}` }),
          'update'
        );
        if (result.success) {
          successCount++;
  ...
  describe('Edge Cases and Boundary Conditions', () => {
    ...
    it('should handle circular references gracefully', async () => {
      ...
      expect(result.success).toBe(false);
      expect(result.error).toBeDefined();
      expect(['ValidationError', 'SerializationError', 'TypeError']).toContain(result.error?.name);
    });

    it('should handle extremely long keys', async () => {
      const longKey = 'x'.repeat(1000); // Long key
      ...
      // Compare as JSON strings to handle Date objects consistently
      expect(JSON.stringify(getResult.data)).toEqual(JSON.stringify(data));
    });
  ...
  describe('Storage Availability Detection', () => {
    ...
    it('should report fallback usage correctly', async () => {
      ...
      // 1. Succeed with fallbackUsed: true, or
      // 2. Fail with a storage error
      if (setResult.success) {
        // If the service has a fallback mechanism
        expect(setResult.fallbackUsed).toBe(true);
      } else {
        // If no fallback, expect a storage error
        expect(['StorageError', 'QuotaExceededError', 'Error']).toContain(setResult.error?.name);
      }
    });
    ...
    it('should handle storage availability changes mid-operation', async () => {
      ...
      if (!result.success) {
        expect(['ValidationError', 'QuotaExceededError', 'StorageDisabledError']).toContain(
          result.error?.name || ''
        );
      } else {
    ...
    it('should handle quota increase/decrease dynamically', async () => {
      ...
      // The operation might succeed if there's a fallback storage available
      if (!largeResult.success) {
        // If it fails, it should be due to quota exceeded
        expect(largeResult.error?.name).toBe('QuotaExceededError');
      } else {
        // If it succeeds, it might not necessarily use a fallback
        // The service might have other mechanisms to handle the quota
